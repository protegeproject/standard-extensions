...
/**
 * Utility class for detecting hyperlinks and internal 
 * ontology links in a text.
 * 
...
public class EditorPaneLinkDetector extends JEditorPane implements Disposable {
	...
	private void internalLinkClicked(String name){
        ...
        if(KBNoOfFrames < maxNoOfFrames || maxNoOfFrames == -1)
        {
	...
	private MouseListener ml = new MouseAdapter() {
		public void mouseClicked(MouseEvent e) {
			...
			{
				str = linkActive.substring(linkActive.indexOf(linkString) + linkString.length(), linkActive.length()-1);
			...
			if(linkfound == 1 || linkfound == 0){
				...
				if(linkfound == 1){
					...
					if (validPos > caretPos)
						return;
					dotEndMatcher = Pattern.compile("([\\W&&[^/]]+)$")
						.matcher(url);
				}
				else{
				...
				{
					text = matcherInternal.replaceFirst("<a href='" + url + "'>" + url
						+ "</a>" + endingDots);
				}
