...
public class JavaCodeGenerator {
    ...
    public void createAll() throws IOException {
        ...
        createFactoryClass();
        printWrappedInstanceClass();
        printFactoryUtilClass();
    }
    ...
    private void printFactoryClass(PrintWriter printWriter) {
        ...
        for (Object element : options.getClses()) {
            ...
            if (!cls.isSystem()) {
	            ...
	            printWriter.println("        Instance inst = kb.getInstance(name);");
	            printWriter.println("        if (inst == null) { return null; }");
	            printWriter.println("        Cls cls = get" + interfaceName + "Class();");
	            printWriter.println("        if (inst.hasType(cls)) {");
	            printWriter.println("            return new " + getImplementationName(cls) + "(inst);");
	            printWriter.println("        }");
	            ...
	            printWriter.println("            result.add(new " + getImplementationName(cls) + "(inst));");
	            printWriter.println("        }");
        ...
        try {
			printAdditionalFactoryMehods(printWriter);
		} catch (IOException e) {
			Log.getLogger().log(Level.WARNING, "Could not write generated code - factory additional methods.\n" +
					"Probably could not find resource FactoryAdditionalMethods.template.", e);
		}
    ...
    private void printWrappedInstanceClass() throws IOException {
		InputStream ins = JavaCodeGenerator.class.getResourceAsStream("resources/AbstractWrappedInstance.template");
		DataInputStream in = new DataInputStream(ins);
	    BufferedReader br = new BufferedReader(new InputStreamReader(in));

		File file = getImplementationFile("AbstractWrappedInstance");
		FileWriter fileWriter = new FileWriter(file);
        PrintWriter printWriter = new PrintWriter(fileWriter);
    ...
    private void printFactoryUtilClass() throws IOException {
		InputStream ins = JavaCodeGenerator.class.getResourceAsStream("resources/FactoryUtil.template");
		DataInputStream in = new DataInputStream(ins);
	    BufferedReader br = new BufferedReader(new InputStreamReader(in));

		File file = getImplementationFile("FactoryUtil");
		FileWriter fileWriter = new FileWriter(file);
        PrintWriter printWriter = new PrintWriter(fileWriter);
